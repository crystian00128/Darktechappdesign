...
function VendedorChat({ currentUsername, currentUserName, currentUserPhoto, clientes, motoristas, onStartCall }: { currentUsername: string; currentUserName?: string; currentUserPhoto?: string; clientes: ChatContact[]; motoristas: ChatContact[]; onStartCall?: (to: string, type: "voice" | "video", toName: string) => void }) {
  ...
  const handleSend = async () => {
    ...
    try { await api.sendMessage(currentUsername, selectedChat, text); await loadMessages(); } catch (err) { console.error("Erro ao enviar:", err); } finally { setSending(false); }
  };
                    ...
                    <GlowCard glowColor={accentColor}>
                      <div className="flex items-center gap-3 p-3.5">
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          <motion.div className="w-2 h-2 bg-[#00ff41] rounded-full"
                            animate={{ opacity: [1, 0.3, 1] }} transition={{ duration: 2, repeat: Infinity }}
                          />
                          <motion.div className="p-2 rounded-lg" style={{ backgroundColor: `${accentColor}15` }}
