...
function MotoristaChat({ currentUsername, currentUserName, currentUserPhoto, vendedorContacts, clienteContacts, onConversationChange, onStartCall }: { currentUsername: string; currentUserName?: string; currentUserPhoto?: string; vendedorContacts: ChatContact[]; clienteContacts: ChatContact[]; onConversationChange?: (open: boolean) => void; onStartCall?: (to: string, type: "voice" | "video", toName: string) => void }) {
  ...
  const handleSend = async () => {
    ...
    try { await api.sendMessage(currentUsername, selectedChat, text); await loadMessages(); } catch (e) { console.error("Erro envio:", e); } finally { setSending(false); }
  };
        ...
        <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} className="fixed inset-0 z-[70] bg-[#050508] flex flex-col">
          ...
          <div className="relative shrink-0 bg-[#0a0a12]/95 backdrop-blur-xl border-b border-[#1f1f2e]/60 px-4 py-3 flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-1.5"><motion.div className="w-1.5 h-1.5 rounded-full bg-[#00ff41]" animate={{ scale: [1, 1.3, 1] }} transition={{ duration: 1.5, repeat: Infinity }} /><span className="text-[#00ff41] text-[11px] font-medium">Online</span></div>
            </div>
          ...
          <div className="relative shrink-0 bg-[#0a0a12]/95 backdrop-blur-xl border-t border-[#1f1f2e]/60 p-3">
            ...
            {isRecording ? (
              ...
              <div className="flex items-center gap-2">
                ...
                <input type="text" value={message} onChange={(e) => setMessage(e.target.value)} onKeyDown={handleKeyDown} onFocus={() => setShowAttachMenu(false)} placeholder="Mensagem..." className="flex-1 px-4 py-3 bg-[#12121a] border border-[#1f1f2e]/60 rounded-xl text-white text-sm focus:outline-none focus:border-[#ff00ff]/40 transition-all placeholder-gray-600" />
                {message.trim() ? (
                    ...
                    <motion.button key={contact.username} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.04 }} onClick={() => { setSelectedChat(contact.username); onConversationChange?.(true); }} whileTap={{ scale: 0.98 }} className="w-full text-left">
                      <GlowCard glowColor={accent}>
                        <div className="flex items-center gap-3 p-3.5">
                          ...
                          <div className="flex-1 min-w-0"><p className="text-white font-semibold text-sm truncate">{contact.name}</p><p className="text-gray-500 text-xs">@{contact.username}</p></div>
                          <div className="flex items-center gap-2 shrink-0">
                            <motion.div className="w-2 h-2 bg-[#00ff41] rounded-full" animate={{ opacity: [1, 0.3, 1] }} transition={{ duration: 2, repeat: Infinity }} />
                            <motion.div className="p-2 rounded-lg" style={{ backgroundColor: `${accent}15` }}><MessageSquare className="w-4 h-4" style={{ color: accent }} /></motion.div>
                          </div>
                        </div>
                      </GlowCard>
                    </motion.button>
